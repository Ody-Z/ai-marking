...
from services.md_to_pdf import MarkdownToPDF
...
class MarkingEngine:
    ...
    async def process_submission(self, criteria_path, homework_path, output_path, 
        ...
        """
        logger.info(f"Processing submission for {student_name or 'Unknown Student'}")
        ...
        try:
            ...
            feedback_md, marks = self.llm_service.generate_feedback(criteria_md, homework_md)
            ...
            logger.info(f"Successfully processed submission, output saved to {output_path}")
            return output_path
            
        except Exception as e:
            logger.error(f"Error processing submission: {str(e)}")
            self._generate_error_report(output_path, str(e), student_name, assignment_title)
            raise
